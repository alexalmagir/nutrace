...
**Version:** 2.0
**Date:** 2026-03-04
**Phase:** D — Build (revised from Phase C)
**Author:** CDO (AI agent)
**Signed off by:** Alexandre Maravilla
**Status:** Approved — updated after Phase D all-hands review (docs/08) and founder risk mitigations (docs/09)

**Change log v1.0 → v2.0 (Phase D all-hands decisions):**
- Screen 5.2 PDF: bar chart SVG → text table (cut for 4-week scope)
- Screen 5.2: added HCP share link option alongside native share
- Screen 6.1: removed symptom timeline table → numbers only (cut for 4-week scope)
- Flow 3: added Frame 3.N3 — symptom nudge notification (new, CPO + CTO commit)
- Design notes updated throughout to reflect Phase D cuts and additions
...
### Notification 3.N3 — Symptom Nudge (no symptoms logged in 2+ days)

**Trigger:** User has logged ≥1 meal entry in the last 2 days but has zero symptom entries in that window. Sent once per 48-hour period. Not sent if user already has a symptom entry today.

**Purpose:** Without symptom data the pattern algorithm cannot compute any correlations. This nudge closes the data gap before the user reaches day 3 with no insight.
...
│  your last meal? Add a      │
│  symptom note — your doctor │
│  needs this data.           │
└─────────────────────────────┘
```

→ Tapping opens Screen 3.1 linked to the most recent meal entry.

**Design notes:**

- Copy is explicit about the clinical purpose ("your doctor needs this data") — this frames logging symptoms as meaningful, not nagging
- Only sent if the user has been actively logging meals but not symptoms — not sent to completely inactive users
- Suppressed if the user has disabled notifications
...
(Bar chart deferred to V1.1 — replaced by text table for MVP scope)
...
→ "Generate doctor link" creates a unique URL (`nutrace.app/share/{token}`) the HCP can open in any browser with no login. User gives explicit consent before the link is generated. Link expires in 7 days, revocable from the app. Footer of the PDF shows: *"Generated by Nutrace · nutrace.app"*.
...
- Numbers shown are from the user's own log — not general population data. No percentages, no statistical language.
...
- **Phase D cut:** Symptom timeline table (per-meal breakdown with dates and emoji) removed. MVP shows counts only. Timeline view deferred to V1.1.
